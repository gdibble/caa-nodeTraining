...
var requestHandler = function requestHandler(request, response) {
  ...
  var queryData    = url.parse(request.url, true).query;
    var at         = queryData && queryData.at;
    var delta      = queryData && Number(queryData.delta);
    var message    = queryData && queryData.message;
  var data;
  ...
  } else if (request.method === 'GET' && request.url.indexOf('/movement') === 0 && at && delta && message) {
    // GET /movement?at=ISODate&delta=0.0&message=Text
    data = {
      at:      at,      // ISODate
      delta:   delta,   // Amount of movement
      message: message  // Text sent via SMS
    };
    if (DEBUG) { console.log('\n* /movement data received', getDate(), '\n   ' + JSON.stringify(data)); }

    if (io.socket) {
       // Give climate data to everyone connected to the web server
      io.socket.broadcast.emit('moved', data);
